...
// ConflictSnapshotReader adapts AdminRepository to conflict.SnapshotReader.
type ConflictSnapshotReader struct {
	Repo *AdminRepository
}

// NewConflictSnapshotReader returns a read-only conflict snapshot reader.
...
func (a ConflictSnapshotReader) ListRoles(ctx context.Context, companyID string) ([]conflict.RoleSnapshot, error) {
	...
	for _, r := range rows {
		out = append(out, roleToConflictSnapshot(r))
	}
	return out, nil
}
...
func roleToConflictSnapshot(r caapp.RoleListItem) conflict.RoleSnapshot {
	return conflict.RoleSnapshot{
		RoleID:      r.RoleID,
		RoleCode:    r.RoleCode,
		RoleName:    r.RoleName,
