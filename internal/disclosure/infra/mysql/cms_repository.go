...
func (r *Repository) listGlobalWorkflowSteps(ctx context.Context, workflowID string) ([]disclosureapp.GlobalWorkflowStepInput, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT step_id, stage, department_id, assignee_role_ids, due_rule, processing_days, display_order
	...
	for rows.Next() {
		...
		if err := rows.Scan(&step.StepID, &step.Stage, &step.DepartmentID, &roleIDsJSON, &step.DueRule, &step.ProcessingDays, &step.DisplayOrder); err != nil {
...
func (r *Repository) UpsertGlobalWorkflow(ctx context.Context, req disclosureapp.CmsUpsertGlobalWorkflowRequest, workflowID string) (*disclosureapp.GlobalWorkflowDTO, error) {
	...
	for i, step := range req.Steps {
		...
		_, err = tx.ExecContext(ctx, `
			...
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
		`, stepID, workflowID, step.Stage, step.DepartmentID, string(roleIDsJSON), step.DueRule, step.ProcessingDays, displayOrder, now)
		if err != nil {
