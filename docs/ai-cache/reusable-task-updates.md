...
- remaining gaps/risks/next steps:
  ...
  - the pre-existing `TestDispatchEmail_SanitisesAllSensitiveVars` failure should be triaged separately (likely needs `support_email` added to the test's `Variables`, or the template's required-var list reviewed) — not addressed here per "không mở rộng scope"
