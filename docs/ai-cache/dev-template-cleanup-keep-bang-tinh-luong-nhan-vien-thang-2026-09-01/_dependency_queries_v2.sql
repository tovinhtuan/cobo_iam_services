...
SELECT 'keep_root' AS q, dt.type_id, dt.status, dt.active_version_no, dtv.name
...
SELECT 'would_delete_versions' AS metric, COUNT(*) AS cnt FROM disclosure_type_versions WHERE type_id <> @keep_id;
SELECT 'would_delete_periodic_cycles' AS metric, COUNT(*) AS cnt FROM periodic_cycles WHERE type_id <> @keep_id;
SELECT 'would_delete_records' AS metric, COUNT(*) AS cnt FROM disclosure_records WHERE type_id <> @keep_id;
SELECT 'would_delete_global_workflows' AS metric, COUNT(*) AS cnt FROM global_workflows WHERE type_id <> @keep_id;
SELECT 'would_delete_global_steps' AS metric, COUNT(*) AS cnt FROM global_workflow_steps WHERE type_id <> @keep_id;
SELECT 'would_delete_company_prefs' AS metric, COUNT(*) AS cnt FROM company_type_preferences WHERE type_id <> @keep_id;
SELECT 'would_delete_template_blocks' AS metric, COUNT(*) AS cnt FROM disclosure_template_blocks WHERE type_id <> @keep_id;
SELECT 'would_delete_display_groups' AS metric, COUNT(*) AS cnt FROM template_display_groups WHERE template_id <> @keep_id;
SELECT 'would_delete_workflow_overrides' AS metric, COUNT(*) AS cnt FROM company_template_workflow_overrides WHERE type_id <> @keep_id;

SELECT 'delete_candidate_submitted_records' AS metric, COUNT(*) AS cnt FROM disclosure_records WHERE type_id <> @keep_id AND submitted_at IS NOT NULL;
SELECT 'delete_candidate_non_draft_records' AS metric, COUNT(*) AS cnt FROM disclosure_records WHERE type_id <> @keep_id AND status <> 'draft';

SELECT 'would_delete_workflow_instances' AS metric, COUNT(DISTINCT wi.workflow_instance_id) AS cnt
FROM workflow_instances wi JOIN disclosure_records dr ON dr.record_id = wi.record_id WHERE dr.type_id <> @keep_id;
...
JOIN disclosure_records dr ON dr.record_id = wi.record_id WHERE dr.type_id <> @keep_id;
