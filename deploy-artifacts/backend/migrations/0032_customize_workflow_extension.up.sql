...
-- Use information_schema guards so reruns after partial apply are safe
-- on MySQL variants that do not support ADD COLUMN IF NOT EXISTS.
-- ============================================================
SET @col_exists = (
  SELECT COUNT(1)
...
SET @col_exists = (
  ...
  WHERE table_schema = DATABASE()
    AND table_name = 'workflow_instances'
    AND column_name = 'workflow_source'
);
SET @sql = IF(
  @col_exists = 0,
